...
| DR-009 | P1 | verified | 全仓测试基线不绿 | 修复 SceneManager 测试与纹理 mock；全仓 `pnpm test` 退出码为 0；不得删除有效断言换取通过 |
...
- **DR-001**：已于 2026-07-21 复验通过并关闭为 `verified`。T1～T6 全部实施完成并经两轮审查修正；角色×资源×操作矩阵见 `dr-001-access-policy.md`，对象存储读取策略见 `dr-001-private-media/spec.md` 与 `deployment-checklist.md`；2026-07-21 于 WSL 内复验全仓 `pnpm test` 退出码 0（Server 141/141、Admin 11/11、Client 36/36、Shared 1/1）且 Server 构建退出码 0。
- **下一待启动项**：DR-002（媒体删除闭环）。注意 DR-001 分支尚未合并回 `main`，合并与生产侧部署动作（禁用 R2 公开访问、CORS、历史迁移）按 `dr-001-private-media/deployment-checklist.md` 执行。
