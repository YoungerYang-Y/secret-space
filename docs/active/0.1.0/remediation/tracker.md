...
| DR-004 | P1 | verified | 旧 Plan 与实际实现失真 | 不修改旧 Plan；在本目录建立 P0～P2 实际状态快照、代码证据和验收结果；后续状态只维护于本追踪表 |
| DR-005 | P1 | verified | 相册模型缺少业务不变量 | 服务端校验模板图片数量、文字要求、回执归属（scope）和页面顺序；异常与并发路径测试通过 |
...
- **DR-001～DR-005 均已验收通过**（verified）；DR-005 实现已随 4 个逻辑提交落地本地 `main`（未推送）。
- **下一待启动项**：按推荐顺序为 P0-GAP-001～004（加载页真实进度、竖屏手势与导航、世界坐标契约、环境音入口）。
- **部署提醒**：生产部署按各 `deployment-checklist.md` 执行：
  ...
  - DR-005: prisma migrate deploy（两个新迁移：Page 唯一序约束、移除冗余索引）
