...
| ID | 优先级 | 状态 | 问题 | 完成标准 |
|---|---|---|---|---|
| DR-001 | P0 | queued | 隐私读取契约冲突 | 在本目录记录角色×资源×操作权限矩阵；内容读取 API 与矩阵一致；鉴权集成测试覆盖匿名、访客、所有者、管理员；明确对象存储读取策略 |
...
- **DR-001**：项目级测试基线已恢复绿色；Spec、Design 与 Plan 已就绪，待从 T1 的 Red 阶段逐步执行。
