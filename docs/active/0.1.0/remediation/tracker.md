...
| DR-002 | P1 | in-progress | 媒体删除无可靠闭环 | 媒体保存稳定 `storageKey`；删除失败可持久化重试；失败有可观测记录；正常与失败路径测试通过 |
...
- **DR-002（in-progress）**：媒体删除无可靠闭环。2026-07-21 启动，现状分析与 Spec 见 `dr-002-media-deletion/`。注意 DR-001 分支尚未合并回 `main`，合并与生产侧部署动作（禁用 R2 公开访问、CORS、历史迁移）按 `dr-001-private-media/deployment-checklist.md` 执行。
